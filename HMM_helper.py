...
def states_to_wordclouds(hmm, obs_map, rhyme_pairs,max_words=50, show=True):
    # Initialize.
    M = 10000
    n_states = len(hmm.A)
    ...
    emission = hmm.generate_sonnet(M,syl_map(obs_map),rhyme_pairs)
...
def common_words_for_states(hmm,obs_map): 
    import numpy as np 
    O = hmm.O
    common_words = []
    for state in range(len(O)): 
        probs = np.array(O[state])
        #probs.argsort()[-10:][::-1]
        words = np.argpartition(probs,-10)[-10:]
...
def syl_map(obs_map): 
    import os
    # Read in syllable map, from words to number of syllables
    # syl = {}
    # with open('./data/Syllable_dictionary.txt') as f: 
    #     for line in f:
    #         cur_line = [elt.strip() for elt in line.split()]
    #         syl[cur_line[0]] = cur_line[1]
    ...
    for line in lines:
        ...
        if line[-1][0].isdigit(): syl[word] = int(line[-1])
        else: syl[word] = int(line[-2])

    # Convert to map from word number (in obs_map) to number of syllables
    syl_map1 = {}
    for key in syl: 
        a = obs_map[key]
        syl_map1[obs_map[key]] = syl[key]
    return syl_map1

def sample_sentence(hmm, obs_map, rhyme_pairs, n_syllables=100):
    # Get reverse map.
    obs_map_r = obs_map_reverser(obs_map)
    syl = syl_map(obs_map)

    # Sample and convert sentence.
    emission, states, n_syls = hmm.generate_sonnet(n_syllables,syl,rhyme_pairs)
    #sentence = [[obs_map_r[i] for i in emission[j]] for j in range(len(emission))]

    return emission# ' '.join(sentence).capitalize() + '...' + str(n_syls)
